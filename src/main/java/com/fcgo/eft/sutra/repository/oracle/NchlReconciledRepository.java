...
public interface NchlReconciledRepository extends JpaRepository<NchlReconciled, String> {
    ...
    @Query(value = "SELECT D.INSTRUCTION_ID FROM EFT_PAYMENT_BATCH_DETAIL D JOIN EFT_PAYMENT_BATCH B ON D.EFT_BATCH_PAYMENT_ID = B.ID LEFT JOIN NCHL_RECONCILED R ON D.INSTRUCTION_ID = R.INSTRUCTION_ID WHERE R.INSTRUCTION_ID IS NULL AND D.NCHL_PUSHED_DATE_TIME <= 20251017000000 AND D.ID NOT LIKE '2510%'", nativeQuery = true)
