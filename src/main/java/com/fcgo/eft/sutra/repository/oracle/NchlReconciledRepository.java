...
public interface NchlReconciledRepository extends JpaRepository<NchlReconciled, String> {
    ...
    @Query(value = "SELECT D.INSTRUCTION_ID FROM EFT_PAYMENT_BATCH_DETAIL D JOIN EFT_PAYMENT_BATCH B ON D.EFT_BATCH_PAYMENT_ID = B.ID LEFT JOIN NCHL_RECONCILED R ON D.INSTRUCTION_ID = R.INSTRUCTION_ID WHERE B.RECEIVE_DATE<251016 AND NCHL_PUSHED_DATE_TIME IS NULL AND R.INSTRUCTION_ID IS NULL", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE EFT_PAYMENT_BATCH_DETAIL SET NCHL_TRANSACTION_TYPE='MNL',NCHL_PUSHED_DATE_TIME='20251017000000',NCHL_CREDIT_STATUS='SENT' WHERE INSTRUCTION_ID=?1", nativeQuery = true)
